...
def get_themes_list():
    ...
    available_themes = yaml.safe_load(open(settings_path))['available_themes']
    return available_themes


def get_levels_list(add_bonuses=None):
    settings_path = '%s/core/config/settings.yml' % config.PROJECT_ROOT
    levels = yaml.safe_load(open(settings_path))['levels']
    level_k = 0
...
def get_pieces_list():
    settings_path = '%s/core/config/settings.yml' % config.PROJECT_ROOT
    available_pieces = yaml.safe_load(open(settings_path))['available_pieces']
